...
// CRC-32/ISO-HDLC of the grammar file: 2642a3f7
...
mod peginator_generated {
    ...
    pub(super) fn parse_ClassStatement<'a, TT: ParseTracer>(
        ...
        let result = {
            let result = ClassStatement_impl::parse(state, global)?
                .map(|r| super::ClassStatement { namespace: r.namespace, body: r.body });
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    ...
    mod ForStatement_impl {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            let ParseOk { result: body, state } =
                parse_Whitespace(state, &mut *global).and_then(|ParseOk { state, .. }| parse_BlockNode(state, &mut *global))?;
            Ok(ParseOk { result: Parsed { pattern, value, body }, state })
        }
        pub struct Parsed {
            pub pattern: PatternNode,
            pub value: ExpressionNode,
    ...
    pub(super) fn parse_ForStatement<'a, TT: ParseTracer>(
        ...
        let result = {
            let result = ForStatement_impl::parse(state, global)?.map(|r| super::ForStatement {
                pattern: r.pattern,
                value: r.value,
                body: r.body,
            });
