...
// CRC-32/ISO-HDLC of the grammar file: e8535904
...
pub type ESCAPE2 = String;
pub type UNICODE4 = String;
pub type UNICODE8 = String;
...
mod peginator_generated {
    ...
    pub(super) fn parse_RawString<'a, TT: ParseTracer>(
        ...
        let result = {
            let result = RawString_impl::parse(state.clone(), global)?
                .map_with_state(|r, new_state| super::RawString { position: state.range_until(new_state) });
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    ...
    mod StringItem_impl {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            ChoiceHelper::new(state)
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_ESCAPE2(state, global))
                        .map_inner(Parsed__override::ESCAPE2)
                })
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_UNICODE4(state, global))
                        .map_inner(Parsed__override::UNICODE4)
                })
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_UNICODE8(state, global))
                        .map_inner(Parsed__override::UNICODE8)
                })
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
    ...
    #[inline]
    pub(super) fn parse_ESCAPE2<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, ESCAPE2> {
        global.tracer.print_trace_start(&state, "ESCAPE2");
        let result = {
            let result = ESCAPE2_impl::parse(state.clone(), global)?.map_with_state(|_, new_state| {
                let string = state.slice_until(new_state).to_string();
                string
            });
            Ok(result)
    ...
    #[inline]
    pub(super) fn parse_UNICODE4<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, UNICODE4> {
        global.tracer.print_trace_start(&state, "UNICODE4");
        let result = {
            let result = UNICODE4_impl::parse(state.clone(), global)?.map_with_state(|_, new_state| {
                let string = state.slice_until(new_state).to_string();
                string
            });
            Ok(result)
    ...
    #[inline]
    pub(super) fn parse_UNICODE8<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, UNICODE8> {
        global.tracer.print_trace_start(&state, "UNICODE8");
        let result = {
            let result = UNICODE8_impl::parse(state.clone(), global)?.map_with_state(|_, new_state| {
                let string = state.slice_until(new_state).to_string();
                string
            });
            Ok(result)
    ...
    mod HEX_impl {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            ChoiceHelper::new(state)
                ...
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_character_range(state, 'a', 'f'))
                        .discard_result()
                })
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
    ...
    pub(super) fn parse_HEX<'a, TT: ParseTracer>(
        ...
        global.tracer.print_trace_start(&state, "HEX");
        let result = {
            let result = HEX_impl::parse(state, global)?.map(|r| super::HEX {});
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
