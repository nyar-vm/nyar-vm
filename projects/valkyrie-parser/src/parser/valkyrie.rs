...
// CRC-32/ISO-HDLC of the grammar file: 8efd2b93
...
pub enum TermNode {
    ...
    MatrixStatement(MatrixStatement),
...
pub struct MatrixStatement {
    pub args: Vec<VectorStatement>,
    pub position: std::ops::Range<usize>,
}
#[derive(Debug, Clone)]
pub struct VectorStatement {
    pub args: Vec<ExpressionNode>,
    pub position: std::ops::Range<usize>,
}
#[derive(Debug, Clone)]
...
mod peginator_generated {
    ...
    mod TermNode_impl {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            ChoiceHelper::new(state)
                ...
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_MatrixStatement(state, global))
                        .map_inner(Parsed__override::MatrixStatement)
                })
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                ...
                mod closure {
                    ...
                    pub fn parse<'a, TT: ParseTracer>(
                        ...
                        let ParseOk { state, .. } = parse_Whitespace(state, &mut *global)
                            ...
                            .discard_result()?;
                        let ParseOk { result: mut args, state } = parse_Whitespace(state, &mut *global)
                            .and_then(|ParseOk { state, .. }| parse_VectorStatement(state, &mut *global))
                            .map_inner(|result| vec![result])?;
                        Ok(ParseOk { result: args, state })
                    }
                    pub type Parsed = Vec<VectorStatement>;
                }
                #[inline(always)]
                pub fn parse<'a, TT: ParseTracer>(
                    state: ParseState<'a>,
                    ...
                    let mut iterations: usize = 0;
                    let mut state = state;
                    let mut args: Vec<VectorStatement> = Vec::new();
                    loop {
                        match closure::parse(state.clone(), global) {
                            Ok(ParseOk { result: __result, state: new_state, .. }) => {
                                args.extend(__result);
    ...
    #[inline]
    pub(super) fn parse_MatrixStatement<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, MatrixStatement> {
        global.tracer.print_trace_start(&state, "MatrixStatement");
        let result = {
            let result = MatrixStatement_impl::parse(state.clone(), global)?
                .map_with_state(|r, new_state| super::MatrixStatement { args: r, position: state.range_until(new_state) });
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    }
    impl PegPosition for MatrixStatement {
        fn position(&self) -> &std::ops::Range<usize> {
            &self.position
        }
    }
    mod VectorStatement_impl {
        use super::*;
        mod part_1 {
            use super::*;
            mod closure {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            let ParseOk { result: mut args, state } = parse_Whitespace(state, &mut *global)
                .and_then(|ParseOk { state, .. }| parse_ExpressionNode(state, &mut *global))
                .map_inner(|result| vec![result])?;
            let ParseOk { result: extend_args_with, state } = part_1::parse(state, global)?;
            args.extend(extend_args_with);
            let ParseOk { state, .. } = parse_Whitespace(state.clone(), &mut *global)
                .and_then(|ParseOk { state, .. }| parse_character_literal(state, ','))
                .discard_result()
    ...
    #[inline]
    pub(super) fn parse_VectorStatement<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, VectorStatement> {
        global.tracer.print_trace_start(&state, "VectorStatement");
        let result = {
            let result = VectorStatement_impl::parse(state.clone(), global)?
                .map_with_state(|r, new_state| super::VectorStatement { args: r, position: state.range_until(new_state) });
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    }
    impl PegPosition for VectorStatement {
        fn position(&self) -> &std::ops::Range<usize> {
            &self.position
        }
    }
