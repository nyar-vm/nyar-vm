...
// CRC-32/ISO-HDLC of the grammar file: eacc3d1d
...
mod peginator_generated {
    ...
    pub(super) fn parse_WhileStatement<'a, TT: ParseTracer>(
        ...
        let result = {
            let result = WhileStatement_impl::parse(state.clone(), global)?.map_with_state(|r, new_state| {
                super::WhileStatement { condition: r.condition, body: r.body, position: state.range_until(new_state) }
            });
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
