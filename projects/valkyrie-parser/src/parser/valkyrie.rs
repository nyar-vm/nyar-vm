// This file was generated by Peginator v0.6.0 built at 1672300246
// CRC-32/ISO-HDLC of the grammar file: 569c96d0
...
mod peginator_generated {
    ...
    mod NamespaceDeclare_impl {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            let ParseOk { result: namespace, state } = parse_Whitespace(state, &mut *global)
                .and_then(|ParseOk { state, .. }| parse_NamespaceDot(state, &mut *global))?;
            Ok(ParseOk { result: namespace, state })
        }
        pub type Parsed = NamespaceDot;
    }
    #[inline]
    ...
    mod LetStatement_impl {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            Ok(ParseOk { result: Parsed { modifiers, name, r#type, value }, state })
        }
        pub struct Parsed {
            pub modifiers: Vec<Modifier>,
            pub name: IdentifierNode,
    ...
    mod ControlFlowNode_impl {
        ...
        mod choice_3 {
            ...
            pub fn parse<'a, TT: ParseTracer>(
                ...
                let ParseOk { state, .. } = parse_Whitespace(state, &mut *global)
                    .and_then(|ParseOk { state, .. }| parse_string_literal(state, "throw"))
                    .discard_result()?;
                let ParseOk { result: value, state } = parse_Whitespace(state, &mut *global)
                    .and_then(|ParseOk { state, .. }| parse_ExpressionNode(state, &mut *global))
                    .map_inner(Some)?;
            ...
            mod closure {
                ...
                pub fn parse<'a, TT: ParseTracer>(
                    ...
                    let ParseOk { state, .. } = parse_Whitespace(state, &mut *global)
                        ...
                        .discard_result()?;
                    let ParseOk { state, .. } = parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_TermNode(state, &mut *global))
                        .discard_result()?;
                    Ok(ParseOk { result: (), state })
                }
                pub type Parsed = ();
    ...
    #[inline]
    pub(super) fn parse_NamespaceDot<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, NamespaceDot> {
        global.tracer.print_trace_start(&state, "NamespaceDot");
        let result = {
            let result = NamespaceDot_impl::parse(state.clone(), global)?
                .map_with_state(|r, new_state| super::NamespaceDot { path: r, position: state.range_until(new_state) });
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    }
    impl PegPosition for NamespaceDot {
        fn position(&self) -> &std::ops::Range<usize> {
            &self.position
        }
    }
