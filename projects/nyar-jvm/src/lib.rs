...
pub fn compile_module_to_jvm(module: &NyarcModule) -> Result<Vec<u8>, JvmAotError> {
    ...
    let idx_valueof_nat = cp_name_and_type(&mut cp, idx_valueof_utf8, idx_valueof_desc_utf8, &mut cp_count);
    ...
    let idx_longvalue_nat = cp_name_and_type(&mut cp, idx_longvalue_utf8, idx_longvalue_desc_utf8, &mut cp_count);
    ...
    let idx_bool_valueof_nat = cp_name_and_type(&mut cp, idx_bool_valueof_utf8, idx_bool_valueof_desc_utf8, &mut cp_count);
    let idx_bool_valueof_mref = cp_methodref(&mut cp, idx_cls_bool, idx_bool_valueof_nat, &mut cp_count);
    ...
    let idx_bool_boolvalue_nat = cp_name_and_type(&mut cp, idx_bool_boolvalue_utf8, idx_bool_boolvalue_desc_utf8, &mut cp_count);
    let idx_bool_boolvalue_mref = cp_methodref(&mut cp, idx_cls_bool, idx_bool_boolvalue_nat, &mut cp_count);
    ...
    let idx_println_nat = cp_name_and_type(&mut cp, idx_println_utf8, idx_println_desc_utf8, &mut cp_count);
    ...
    let idx_concat_nat = cp_name_and_type(&mut cp, idx_concat_utf8, idx_concat_desc_utf8, &mut cp_count);
    ...
    let idx_length_nat = cp_name_and_type(&mut cp, idx_length_utf8, idx_length_desc_utf8, &mut cp_count);
    ...
    let idx_getbytes_nat = cp_name_and_type(&mut cp, idx_getbytes_utf8, idx_getbytes_desc_utf8, &mut cp_count);
    ...
    let idx_paths_get_desc_utf8 = cp_utf8(&mut cp, "(Ljava/lang/String;[Ljava/lang/String;)Ljava/nio/file/Path;", &mut cp_count);
    let idx_paths_get_nat = cp_name_and_type(&mut cp, idx_paths_get_utf8, idx_paths_get_desc_utf8, &mut cp_count);
    ...
    let idx_files_read_nat = cp_name_and_type(&mut cp, idx_files_read_utf8, idx_files_read_desc_utf8, &mut cp_count);
    ...
    let idx_files_write_desc_utf8 = cp_utf8(&mut cp, "(Ljava/nio/file/Path;[B[Ljava/nio/file/OpenOption;)Ljava/nio/file/Path;", &mut cp_count);
    let idx_files_write_nat = cp_name_and_type(&mut cp, idx_files_write_utf8, idx_files_write_desc_utf8, &mut cp_count);
    ...
    let idx_substring_nat = cp_name_and_type(&mut cp, idx_substring_utf8, idx_substring_desc_utf8, &mut cp_count);
    ...
    let idx_charat_nat = cp_name_and_type(&mut cp, idx_charat_utf8, idx_charat_desc_utf8, &mut cp_count);
    ...
    let idx_equals_nat = cp_name_and_type(&mut cp, idx_equals_utf8, idx_equals_desc_utf8, &mut cp_count);
    ...
    let idx_tostring_nat = cp_name_and_type(&mut cp, idx_tostring_utf8, idx_tostring_desc_utf8, &mut cp_count);
        ...
        for ins in &instrs {
            if let Instruction::Push(idx) = ins {
                if let Some(Constant::Int(v)) = module.constants.get(*idx as usize) {
                    long_indices.entry(*v).or_insert_with(|| cp_long(&mut cp, *v, &mut cp_count));
                ...
                if let Some(Constant::String(s)) = module.constants.get(*idx as usize) {
                 ...
                 long_indices.entry(*class_idx as i64).or_insert_with(|| cp_long(&mut cp, *class_idx as i64, &mut cp_count));
            }
        }
    }
        ...
        if let Some(trait_info) = module.traits.get(impl_info.trait_idx as usize) {
            for (i, method_name) in trait_info.methods.iter().enumerate() {
                if i < impl_info.methods.len() {
                    ...
                    method_impls.entry(method_name.clone())
                        .or_default()
    ...
    for (name, argc) in &sorted_dispatches {
        ...
        let mut desc = String::new();
        desc.push('(');
        for _ in 0..(*argc + 1) { // receiver + args
            desc.push_str("Ljava/lang/Object;");
        }
        desc.push_str("Ljava/lang/Object;"); // closure context/extra
        desc.push(')');
        ...
        for (i_idx, ins) in instrs.into_iter().enumerate() {
            ...
            match ins {
                Instruction::Push(idx) => match module.constants.get(idx as usize) {
                    ...
                    Some(_) => return Err(JvmAotError::UnsupportedOpcode("Push-unsupported".to_string())),
                    None => return Err(JvmAotError::Decode("const out of range".to_string())),
                },
                Instruction::Pop => { code.push(0x57); } // pop
                Instruction::Dup(n) => {
                     if n == 0 {
                         code.push(0x59); // dup
                     } else {
                ...
                Instruction::I64Add | Instruction::I64Sub | Instruction::I64Mul | Instruction::I64DivS | Instruction::I64RemS => {
                    code.push(0x5F); // swap -> B A
                    code.push(0xC0); code.extend_from_slice(&idx_cls_long.to_be_bytes());
                    code.push(0xB6); code.extend_from_slice(&idx_longvalue_mref.to_be_bytes());
                    // Stack: B long(A)
                    ...
                    code.push(op);
                    code.push(0xB8); code.extend_from_slice(&idx_valueof_mref.to_be_bytes());
                }
                ...
                Instruction::GetField(name_idx) => {
                    ...
                    let map = field_impls.get(field_name).ok_or_else(|| {
                        JvmAotError::UnsupportedOpcode(format!("GetField: field {} has no layout", field_name))
                    })?;
                ...
                Instruction::SetField(name_idx) => {
                    ...
                    let map = field_impls.get(field_name).ok_or_else(|| {
                        JvmAotError::UnsupportedOpcode(format!("SetField: field {} has no layout", field_name))
                    })?;
                    let tmp_arr_local: u8 = (ch.locals + 1) as u8;
                ...
                Instruction::FFICall(desc, argc) => {
                    let name = module.constants.get(desc as usize)
                        .and_then(|c| if let Constant::String(s) = c { Some(s.as_str()) } else { None })
                    ...
                    match (name, argc) {
                        ...
                        ("print", 1) => {
                            code.push(0xB2); code.extend_from_slice(&idx_out_fref.to_be_bytes());
                            code.push(0x5F);
                            code.push(0xB6); code.extend_from_slice(&idx_println_mref.to_be_bytes());
                            code.push(0x01);
                        }
                        ("read_file", 1) => {
                            code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
                            code.push(0x03);
                            code.push(0xBD);
                            code.extend_from_slice(&idx_cls_str.to_be_bytes());
                            code.push(0xB8); code.extend_from_slice(&idx_paths_get_mref.to_be_bytes());
                            code.push(0xB8); code.extend_from_slice(&idx_files_read_mref.to_be_bytes());
                        ...
                        ("write_file", 2) => {
                            code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
                            code.push(0x13); code.extend_from_slice(&idx_utf8_str.to_be_bytes());
                            code.push(0xB6); code.extend_from_slice(&idx_getbytes_mref.to_be_bytes());
                            code.push(0x5F);
                            code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
                            code.push(0x03); code.push(0xBD); code.extend_from_slice(&idx_cls_str.to_be_bytes());
                            code.push(0xB8); code.extend_from_slice(&idx_paths_get_mref.to_be_bytes());
                            code.push(0x5F);
                            code.push(0x03); code.push(0xBD); code.extend_from_slice(&idx_cls_openopt.to_be_bytes());
                            code.push(0xB8); code.extend_from_slice(&idx_files_write_mref.to_be_bytes());
                            code.push(0x57);
                            code.push(0x01);
                        }
                        ("len", 1) => {
                            let mref = dispatch_mref_map.get(&("len".to_string(), 0))
                                .ok_or_else(|| JvmAotError::UnsupportedOpcode("len dispatch not found".to_string()))?;
                        ...
                        ("eq", 2) => {
                            ...
                            code.push(0xB6); code.extend_from_slice(&idx_equals_mref.to_be_bytes());
                            code.push(0xB8); code.extend_from_slice(&idx_bool_valueof_mref.to_be_bytes());
                        }
                        ("chars", 1) => {
                            code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
                        }
                        ...
                        ("get", 2) => {
                             ...
                             code.push(0x60); // iadd
                             code.push(0xB6); code.extend_from_slice(&idx_substring_mref.to_be_bytes());
                         }
                        ("ord", 1) => {
                             code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
                             code.push(0x03); // iconst_0
                             code.push(0xB6); code.extend_from_slice(&idx_charat_mref.to_be_bytes());
                             code.push(0x85); // i2l
                             code.push(0xB8); code.extend_from_slice(&idx_valueof_mref.to_be_bytes());
                        }
                         _ => return Err(JvmAotError::UnsupportedOpcode(format!("FFICall({},{})", name, argc))),
                    }
                }
                Instruction::Call(target_idx, argc) => {
                    let callee = module.chunks.get(target_idx as usize)
                    ...
                    for _ in 0..need_pad { code.push(0x01); } // push null
                ...
                Instruction::InvokeMethod(name_idx, argc) => {
                    let name = module.constants.get(name_idx as usize)
                        .and_then(|c| if let Constant::String(s) = c { Some(s.as_str()) } else { None })
                        .ok_or(JvmAotError::Decode("method name".to_string()))?;
                    let mref = dispatch_mref_map.get(&(name.to_string(), argc))
                        .ok_or_else(|| JvmAotError::UnsupportedOpcode(format!("InvokeMethod: {} not found", name)))?;
                ...
                Instruction::Return => { code.push(0xB0); } // areturn
                _ => return Err(JvmAotError::UnsupportedOpcode(format!("{:?}", ins))),
            }
        }
        
        for (pos, target_idx) in branches.iter().copied() {
             if target_idx >= ins_offsets.len() { return Err(JvmAotError::Decode("branch out of range".to_string())); }
    ...
    for (name, argc) in &sorted_dispatches {
        ...
        for (off_pos, chunk_idx) in case_offsets {
             ...
             for _ in 0..need_pad { code.push(0x01); } // padding
        ...
        if name == "len" && *argc == 0 {
             code.push(0x2A); 
             code.push(0xC1); code.extend_from_slice(&idx_cls_str.to_be_bytes());
             code.push(0x99); let p = code.len(); code.extend_from_slice(&0u16.to_be_bytes());
             code.push(0x2A); code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
             code.push(0xB6); code.extend_from_slice(&idx_length_mref.to_be_bytes());
             code.push(0x85); code.push(0xB8); code.extend_from_slice(&idx_valueof_mref.to_be_bytes());
             code.push(0xB0);
             let t = code.len();
             let r = (t as isize - (p as isize - 1)) as i16;
             code[p] = r.to_be_bytes()[0]; code[p+1] = r.to_be_bytes()[1];

             if let Some(map) = field_impls.get("len") {
                 let tmp_arr_local = (*argc + 2) as u8;
                 code.push(0x2A);
                 code.push(0x3A); code.push(tmp_arr_local);
                 code.push(0x19); code.push(tmp_arr_local);
                 code.push(0xC0); code.extend_from_slice(&idx_class_obj_arr.to_be_bytes());
                 code.push(0x3A); code.push(tmp_arr_local);
                 code.push(0x19); code.push(tmp_arr_local);
                 code.push(0x03); code.push(0x32);
                 code.push(0xC0); code.extend_from_slice(&idx_cls_long.to_be_bytes());
                 code.push(0xB6); code.extend_from_slice(&idx_longvalue_mref.to_be_bytes());
                 code.push(0x88);
                 code.push(0xAB);
            ...
            code.push(0x2A); code.push(0x2B);
            code.push(0xB6); code.extend_from_slice(&idx_equals_mref.to_be_bytes());
            code.push(0xB8); code.extend_from_slice(&idx_bool_valueof_mref.to_be_bytes());
            code.push(0xB0);
        } else if (name == "lt" || name == "gt" || name == "le" || name == "ge") && *argc == 1 {
            code.push(0x2A); code.push(0xC0); code.extend_from_slice(&idx_cls_long.to_be_bytes());
            code.push(0xB6); code.extend_from_slice(&idx_longvalue_mref.to_be_bytes());
            code.push(0x2B); code.push(0xC0); code.extend_from_slice(&idx_cls_long.to_be_bytes());
            code.push(0xB6); code.extend_from_slice(&idx_longvalue_mref.to_be_bytes());
            code.push(0x94); // lcmp
            ...
            code.push(op);
            let pos_succ = code.len(); code.extend_from_slice(&0u16.to_be_bytes());
            code.push(0x03); // iconst_0
            code.push(0xA7); // goto end
            let pos_end = code.len(); code.extend_from_slice(&0u16.to_be_bytes());
            ...
            code[pos_succ] = rel.to_be_bytes()[0]; code[pos_succ+1] = rel.to_be_bytes()[1];
            let rel = (pos_after as isize - (pos_end as isize - 1)) as i16;
            code[pos_end] = rel.to_be_bytes()[0]; code[pos_end+1] = rel.to_be_bytes()[1];
            ...
            // Check String
            code.push(0x2A); code.push(0xC1); code.extend_from_slice(&idx_cls_str.to_be_bytes());
            code.push(0x99); let p_not_str = code.len(); code.extend_from_slice(&0u16.to_be_bytes());
            
            // String concat
            code.push(0x2A); code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
            code.push(0x2B); code.push(0xC0); code.extend_from_slice(&idx_cls_str.to_be_bytes());
            code.push(0xB6); code.extend_from_slice(&idx_concat_mref.to_be_bytes());
            code.push(0xB0);
            
            let t = code.len();
            let r = (t as isize - (p_not_str as isize - 1)) as i16;
            code[p_not_str] = r.to_be_bytes()[0]; code[p_not_str+1] = r.to_be_bytes()[1];
            
            // Check Long
            code.push(0x2A); code.push(0xC1); code.extend_from_slice(&idx_cls_long.to_be_bytes());
            code.push(0x99); let p_not_long = code.len(); code.extend_from_slice(&0u16.to_be_bytes());
            ...
            code[p_not_long] = r.to_be_bytes()[0]; code[p_not_long+1] = r.to_be_bytes()[1];
    ...
    // Main stub
    {
        ...
        for _ in 0..main_chunk.locals { code.push(0x01); } // aconst_null
