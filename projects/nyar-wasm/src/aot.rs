...
use nyar_vm::bytecode::format::{Chunk, Constant, NyarcModule};
use wasm_encoder::{
    CodeSection, ConstExpr, DataSection, ElementSection, Elements, ExportKind, ExportSection,
    Function, FunctionSection, GlobalSection, GlobalType, ImportSection, Instruction as WasmInst,
...
impl Default for WasmCompileOptions {
    fn default() -> Self {
        Self {
            strict_closure_arity_check: false,
            closure_type_selection_by: ClosureTypeSelectionMode::ByCallsite,
        }
    }
}

impl AotCompiler for WasmBackend {
...
pub fn compile_module_to_wasm_with(
    module: &NyarcModule,
    ...
    if options.strict_closure_arity_check {
        for call_a in &closure_call_arities {
            if !closure_arities.contains(call_a) {
                return Err(WasmAotError::Decode(format!(
                    "closure call arity {} not in target arities {:?}",
                    call_a, closure_arities
                )));
            }
        }
    }
    ...
    let mut next_type_index = 0u32;
    {
        ...
        for ch in &module.chunks {
            func_arities.insert(ch.locals as u32);
        }
        for &a in closure_arities.iter() {
            func_arities.insert(a as u32);
        }
    ...
    for (name, arities) in &ffi_arities {
        for &arity in arities {
            let ty = ffi_arity_type_index
                .get(&arity)
                .copied()
                .ok_or_else(|| WasmAotError::Decode("ffi arity type missing".to_string()))?;
    ...
    table.table(TableType {
        ...
        minimum: module.chunks.len() as u32,
        maximum: None,
    });
    m.section(&table);
    ...
    for i in 0..module.chunks.len() {
        func_indices.push(import_count + (i as u32));
    }
    ...
    for ch in &module.chunks {
        let ty = func_arity_type_index
            .get(&(ch.locals as u32))
            .copied()
            .ok_or_else(|| WasmAotError::Decode("chunk arity type missing".to_string()))?;
    ...
    for ch in &module.chunks {
        let body = compile_chunk(
            module,
            ch,
            import_count,
            &func_arity_type_index,
            &ffi_import_indices,
        )?;
        code.function(&body);
    }
...
fn compile_chunk(
    ...
    ch: &Chunk,
    import_count: u32,
    func_arity_type_index: &std::collections::HashMap<u32, u32>,
    ffi_import_indices: &std::collections::HashMap<(String, u8), u32>,
    ...
    for (i_idx, ins) in instrs.into_iter().enumerate() {
        ...
        match ins {
            ...
            Instruction::MakeClosure(func_idx, ref upvalues) => {
                ...
                func.instruction(&WasmInst::I32Store(MemArg {
                    offset: 0,
                    align: 2,
                    memory_index: 0,
                }));
                ...
                for uv in upvalues.iter() {
                    ...
                    if uv.is_local {
                        ...
                        func.instruction(&WasmInst::I64Load(MemArg {
                            offset: src_off,
                            align: 3,
                            memory_index: 0,
                        }));
                    }
                    func.instruction(&WasmInst::I64Store(MemArg {
                        offset: off,
                        align: 3,
                        memory_index: 0,
                    }));
            ...
            Instruction::CallClosure(argc) => {
                ...
                func.instruction(&WasmInst::LocalGet(env_tmp_local_index));
                func.instruction(&WasmInst::LocalGet(env_tmp_local_index));
                func.instruction(&WasmInst::I32Load(MemArg {
                    offset: 0,
                    align: 2,
                    memory_index: 0,
                }));
                let ty_idx = *func_arity_type_index.get(&(argc as u32)).ok_or_else(|| {
                    WasmAotError::Decode("closure arity type missing".to_string())
                })?;
            ...
            Instruction::Jump(off) if off > 0 => {
                if let Some(&end) = forward_end_map.get(&i_idx) {
                    ...
                    return Err(WasmAotError::UnsupportedOpcode(
                        "Jump-negative-or-invalid".to_string(),
                    ));
                }
            }
            Instruction::Jump(off) if off < 0 => {
                ...
                if target == 0 {
                    ...
                    return Err(WasmAotError::UnsupportedOpcode(
                        "Backward jump to non-zero target".to_string(),
                    ));
                }
            }
            Instruction::JumpIfFalse(off) if off > 0 => {
                if let Some(&end) = forward_end_map.get(&i_idx) {
                    ...
                    return Err(WasmAotError::UnsupportedOpcode(
                        "JumpIfFalse-negative-or-invalid".to_string(),
                    ));
                }
            }
            Instruction::JumpIfFalse(off) if off < 0 => {
                ...
                if target == 0 {
                    ...
                    return Err(WasmAotError::UnsupportedOpcode(
                        "Backward JumpIfFalse non-zero".to_string(),
                    ));
                }
            }
            Instruction::JumpIfNull(off) if off > 0 => {
                if let Some(&end) = forward_end_map.get(&i_idx) {
                    ...
                    return Err(WasmAotError::UnsupportedOpcode(
                        "JumpIfNull-negative-or-invalid".to_string(),
                    ));
                }
            }
            Instruction::JumpIfNull(off) if off < 0 => {
                ...
                if target == 0 {
                    ...
                    return Err(WasmAotError::UnsupportedOpcode(
                        "Backward JumpIfNull non-zero".to_string(),
                    ));
                }
            }
            Instruction::Call(chunk_idx, argc) => {
                ...
                let callee = module
                    ...
                    .ok_or_else(|| WasmAotError::Decode("callee out of range".to_string()))?;
                if (argc as u32) > (callee.locals as u32) {
                    return Err(WasmAotError::UnsupportedOpcode(format!(
                        "Call argc {} exceeds callee arity {}",
                        argc, callee.locals
                    )));
                }
                // pad missing args with zeros to callee arity
                let need_pad = (callee.locals as i32) - (argc as i32);
                for _ in 0..need_pad.max(0) {
            ...
            Instruction::WithHandler(_) => {
                // degrade: no-op in AOT
            }
