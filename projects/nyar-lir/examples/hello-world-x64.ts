...
function createHelloWorldExe() {
    ...
    const code_length = hello_code.length; // 汇编代码的长度
    ...
    // 当前指令的下一条指令的地址是 hello_code[7] + 7 = 14
    const rip_offset_to_title = (text_section.get_virtual_address() + offset_to_title) - (text_section.get_virtual_address() + 7 + 7);
    fill_offset(hello_code, 10, rip_offset_to_title);

    // lea r8, [rip + offset_to_message]
    // 当前指令的下一条指令的地址是 hello_code[14] + 7 = 21
    const rip_offset_to_message = (text_section.get_virtual_address() + offset_to_message) - (text_section.get_virtual_address() + 14 + 7);
    ...
    // 当前指令的下一条指令的地址是 hello_code[33] + 6 = 39
    const rip_offset_to_exit_process_iat = exit_process_iat_rva - (text_section.get_virtual_address() + 33 + 6);
    fill_offset(hello_code, 35, rip_offset_to_exit_process_iat);
