...
impl CodeGenerator {
    ...
    fn infer_type(&self, expr: &Expression) -> GaiaType {
        match expr {
            ...
            Expression::FieldAccess { object, field } => {
                ...
                let struct_name = match obj_type {
                    GaiaType::Object => {
                        match object.as_ref() {
                            Expression::Identifier(name) => {
                                self.variable_types.get(name).cloned().unwrap_or_else(|| "Object".to_string())
                            }
                ...
                if let Expression::FieldAccess { object: inner_obj, field: inner_field } = object.as_ref() {
                    ...
                    if let Some(inner_struct_def) = self.structs.get(&inner_struct_name) {
                        if let Some(inner_field_def) = inner_struct_def.fields.iter().find(|f| f.name == *inner_field) {
                            if let Type::Custom(nest_struct_name) = &inner_field_def.field_type {
                                if let Some(nest_struct_def) = self.structs.get(nest_struct_name) {
                                    if let Some(f_def) = nest_struct_def.fields.iter().find(|f| f.name == *field) {
            ...
            Expression::FunctionCall { .. } | Expression::MethodCall { .. } | Expression::MacroCall { .. } => {
    ...
    fn get_struct_name(&self, expr: &Expression) -> String {
        match expr {
            Expression::Identifier(name) => self.variable_types.get(name).cloned().unwrap_or_else(|| "Object".to_string()),
            Expression::FieldAccess { object, field } => {
    ...
    pub fn generate(&mut self, program: &Program) -> Result<GaiaModule> {
        ...
        for struct_def in &program.structs {
            ...
            structs.push(GaiaStruct { name: struct_def.name.clone(), fields });
        }
    ...
    fn generate_function(&mut self, function: &Function) -> Result<GaiaFunction> {
        ...
        for (i, param) in function.parameters.iter().enumerate() {
            ...
            self.variable_gaia_types.insert(param.name.clone(), gaia_type.clone());
        ...
        // 处理返回值
        let return_type = function.return_type.as_ref().map(|t| t.to_gaia_type()).unwrap_or(GaiaType::Void);

        // 如果函数没有显式返回，添加默认返回
        // 在新版中，Ret 是 CoreInstruction
        if !instructions.iter().any(|inst| matches!(inst, GaiaInstruction::Core(CoreInstruction::Ret))) {
            // 特殊处理 main 函数：如果推断返回 i32，则压入 0
            if function.name == "main" && return_type == GaiaType::Void {
                // main 默认返回 0
                instructions.push(GaiaInstruction::Core(CoreInstruction::PushConstant(GaiaConstant::I32(0))));
            }
    ...
    fn generate_statement(&mut self, statement: &Statement, instructions: &mut Vec<GaiaInstruction>) -> Result<()> {
        match statement {
            ...
            Statement::VariableDeclaration { name, var_type, initializer, is_mutable } => {
                ...
                if let Some(init_expr) = initializer {
                    ...
                    if let Expression::StructInstantiation { name: struct_name, .. } = init_expr {
                ...
                // 存储到局部变量
                instructions.push(GaiaInstruction::Core(CoreInstruction::StoreLocal(index as u32, gaia_type)));
            }
            ...
            Statement::If { condition, then_branch, else_branch } => {
                ...
                // 如果为假，跳转到 else 或 end
                instructions.push(GaiaInstruction::Core(CoreInstruction::BrFalse(if else_branch.is_some() {
                    else_label.clone()
                }
                else {
                    end_label.clone()
            ...
            Statement::For { var_name, iterable, body } => {
                ...
                if let Expression::Range { start, end } = iterable {
                    ...
                    self.variable_gaia_types.insert(var_name.clone(), var_type.clone());
                    instructions.push(GaiaInstruction::Core(CoreInstruction::StoreLocal(index as u32, var_type.clone())));

                    // 2. 循环开始标签
                    instructions.push(GaiaInstruction::Core(CoreInstruction::Label(start_label.clone())));

                    // 3. 检查条件 (i < end)
                    instructions.push(GaiaInstruction::Core(CoreInstruction::LoadLocal(index as u32, var_type.clone())));
                    self.generate_expression(end, instructions)?;
                    instructions.push(GaiaInstruction::Core(CoreInstruction::Cmp(CmpCondition::Lt, var_type.clone())));
                    instructions.push(GaiaInstruction::Core(CoreInstruction::BrFalse(end_label.clone())));
                    ...
                    // 5. 递增变量 (i = i + 1)
                    instructions.push(GaiaInstruction::Core(CoreInstruction::LoadLocal(index as u32, var_type.clone())));
                    instructions.push(GaiaInstruction::Core(CoreInstruction::PushConstant(GaiaConstant::I32(1))));
                    instructions.push(GaiaInstruction::Core(CoreInstruction::Add(var_type.clone())));
                    instructions.push(GaiaInstruction::Core(CoreInstruction::StoreLocal(index as u32, var_type)));
                ...
                match target {
                    Expression::Identifier(name) => {
                        ...
                        if !self.mutable_vars.contains(name) {
                            return Err(GaiaError::syntax_error(format!("不能对不可变变量赋值: {}", name), SourceLocation::default()));
                        }
                        ...
                        // 存储
                        let gaia_type = self.variable_gaia_types.get(name).cloned().unwrap_or(GaiaType::I32);
                        if let Some(&index) = self.parameters.get(name) {
                            instructions.push(GaiaInstruction::Core(CoreInstruction::StoreArg(index as u32, gaia_type)));
                        }
                    ...
                    Expression::FieldAccess { object, field } => {
                        ...
                        instructions.push(GaiaInstruction::Core(CoreInstruction::StoreField(struct_name, field.clone())));
                    }
                    Expression::IndexAccess { object, index } => {
                        ...
                        instructions.push(GaiaInstruction::Core(CoreInstruction::StoreElement(elem_type)));
                    }
                    _ => return Err(GaiaError::syntax_error("无效的赋值目标".to_string(), SourceLocation::default())),
                }
            }
        }
        Ok(())
    ...
    fn generate_expression(&mut self, expression: &Expression, instructions: &mut Vec<GaiaInstruction>) -> Result<()> {
        match expression {
            ...
            Expression::Identifier(name) => {
                let gaia_type = self.variable_gaia_types.get(name).cloned().unwrap_or(GaiaType::I32);
                if let Some(&index) = self.parameters.get(name) {
                    instructions.push(GaiaInstruction::Core(CoreInstruction::LoadArg(index as u32, gaia_type)));
                }
                else if let Some(&index) = self.locals.get(name) {
                    instructions.push(GaiaInstruction::Core(CoreInstruction::LoadLocal(index as u32, gaia_type)));
                }
                else {
                    return Err(GaiaError::syntax_error(format!("未定义的变量: {}", name), SourceLocation::default()));
                }
            ...
            Expression::BinaryOperation { left, operator, right } => {
                ...
                let instruction = match operator {
                    ...
                    BinaryOperator::Subtract => GaiaInstruction::Core(CoreInstruction::Sub(left_type)),
                    BinaryOperator::Multiply => GaiaInstruction::Core(CoreInstruction::Mul(left_type)),
                    BinaryOperator::Divide => GaiaInstruction::Core(CoreInstruction::Div(left_type)),
                    BinaryOperator::Equal => GaiaInstruction::Core(CoreInstruction::Cmp(CmpCondition::Eq, left_type)),
                    BinaryOperator::NotEqual => GaiaInstruction::Core(CoreInstruction::Cmp(CmpCondition::Ne, left_type)),
                    BinaryOperator::Less => GaiaInstruction::Core(CoreInstruction::Cmp(CmpCondition::Lt, left_type)),
                    BinaryOperator::LessEqual => GaiaInstruction::Core(CoreInstruction::Cmp(CmpCondition::Le, left_type)),
                    BinaryOperator::Greater => GaiaInstruction::Core(CoreInstruction::Cmp(CmpCondition::Gt, left_type)),
                    BinaryOperator::GreaterEqual => GaiaInstruction::Core(CoreInstruction::Cmp(CmpCondition::Ge, left_type)),
                    BinaryOperator::And => GaiaInstruction::Core(CoreInstruction::And(GaiaType::Bool)),
            ...
            Expression::FunctionCall { name, arguments } => {
                ...
                instructions.push(GaiaInstruction::Core(CoreInstruction::Call(name.clone(), arguments.len())));
            }
            Expression::MethodCall { object, method, arguments } => {
                if let Expression::Identifier(obj_name) = object.as_ref() {
                    if obj_name == "console" && method == "log" {
                        ...
                        instructions.push(GaiaInstruction::Core(CoreInstruction::Call("console.log".to_string(), arguments.len())));
                        return Ok(());
                    }
                }
            ...
            Expression::MacroCall { name, arguments } => {
                if name == "println" {
                    ...
                    instructions.push(GaiaInstruction::Core(CoreInstruction::Call("println".to_string(), arguments.len())));
                }
                else {
            ...
            Expression::StructInstantiation { name, fields } => {
                ...
                for (field_name, value_expr) in fields {
                    ...
                    instructions.push(GaiaInstruction::Core(CoreInstruction::StoreField(name.clone(), field_name.clone())));
                }
            }
            Expression::FieldAccess { object, field } => {
                self.generate_expression(object, instructions)?;
                let struct_name = self.get_struct_name(object);
                instructions.push(GaiaInstruction::Core(CoreInstruction::LoadField(struct_name, field.clone())));
            }
            Expression::Range { .. } => {
                return Err(GaiaError::syntax_error("范围表达式仅支持在 for 循环中使用".to_string(), SourceLocation::default()));
            }
            Expression::ArrayLiteral(elements) => {
                let elem_type = if let Some(first) = elements.first() { self.infer_type(first) } else { GaiaType::I32 };

                instructions.push(GaiaInstruction::Core(CoreInstruction::PushConstant(GaiaConstant::I32(elements.len() as i32))));
                instructions.push(GaiaInstruction::Core(CoreInstruction::NewArray(elem_type.clone())));

                for (i, expr) in elements.iter().enumerate() {
                    instructions.push(GaiaInstruction::Core(CoreInstruction::Dup));
                    instructions.push(GaiaInstruction::Core(CoreInstruction::PushConstant(GaiaConstant::I32(i as i32))));
                    self.generate_expression(expr, instructions)?;
                    instructions.push(GaiaInstruction::Core(CoreInstruction::StoreElement(elem_type.clone())));
                }
            }
            Expression::IndexAccess { object, index } => {
                self.generate_expression(object, instructions)?;
                ...
                instructions.push(GaiaInstruction::Core(CoreInstruction::LoadElement(elem_type)));
            }
        }
        Ok(())
    }
