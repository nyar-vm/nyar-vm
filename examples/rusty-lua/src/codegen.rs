...
use chomsky_uir::{IKunTree, IntentBuilder, EGraph, IKun, ConstraintAnalysis};
...
impl GaiaTranslator {
    ...
    pub fn translate_to_tree(&self, _ast: &LuaRoot) -> Result<IKunTree, NyarError> {
        ...
        let root_id = builder.extension("lua_module", vec![builder.string("mini_lua_program", loc)], loc);
