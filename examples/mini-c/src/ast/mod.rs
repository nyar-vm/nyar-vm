...
pub enum Declaration {
    /// 函数声明
    Function { return_type: Type, name: String, parameters: Vec<Parameter>, body: Option<CompoundStatement> },
    /// 变量声明
    Variable { type_: Type, name: String, initializer: Option<Expression> },
    /// 结构体声明
...
pub enum Statement {
    ...
    /// for 循环
    For { init: Option<Expression>, condition: Option<Expression>, update: Option<Expression>, body: Box<Statement> },
    /// break 语句
    Break,
    /// continue 语句
    Continue,
    /// switch 语句
    Switch { expression: Expression, body: Box<Statement> },
    /// case 标签
    Case { value: Expression, statement: Box<Statement> },
    /// default 标签
    Default(Box<Statement>),
    /// goto 语句
    Goto(String),
    /// 标签语句
    Label { name: String, statement: Box<Statement> },
    /// 变量声明语句
...
pub enum Expression {
    ...
    /// 指针成员访问 (->)
    PointerAccess { pointer: Box<Expression>, member: String },
    /// 条件表达式 (三元运算符)
    Conditional { condition: Box<Expression>, true_expr: Box<Expression>, false_expr: Box<Expression> },
    /// sizeof 表达式
    Sizeof(Box<Expression>),
    /// 类型转换
    Cast { type_: Box<Type>, expression: Box<Expression> },
    /// 逗号表达式
...
pub enum Type {
    ...
    Pointer(Box<Type>),
    /// 数组类型
    Array { element_type: Box<Type>, size: Option<Box<Expression>> },
    /// 函数类型
...
impl Declaration {
    ...
    pub fn function(
        ...
        Self::Function { return_type, name: name.into(), parameters, body }
    }

    /// 创建一个变量声明
    pub fn variable(type_: Type, name: impl Into<String>, initializer: Option<Expression>) -> Self {
        Self::Variable { type_, name: name.into(), initializer }
    }
}
